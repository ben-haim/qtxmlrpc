...
SslParams_::SslParams_ ( const QString &certFile, const QString &keyFile, const QByteArray &passwd, QObject *parent ) :
    QObject( parent )
{
    if ( !certFile.isEmpty() )
      {
...
HttpsClient::HttpsClient( const QString &host, const quint16 port, const QString &path,
                          const HttpClient::HttpMethod method, const QString &cert, const QString &key,
                          const QByteArray &passwd ) :
    HttpClient( host, port, path, method ),
    ssl( cert, key, passwd, this )
{ }
...
QAbstractSocket *HttpsClient::buildSocket()
{
    ...
    if ( !ssl.certificate.isNull() ) socket->setLocalCertificate( ssl.certificate );
    if ( !ssl.privateKey.isNull() ) socket->setPrivateKey( ssl.privateKey );
...
{
    if ( qobject_cast < QSslSocket * > (socket)->isEncrypted() ) HttpClient::protocolStart();
}
