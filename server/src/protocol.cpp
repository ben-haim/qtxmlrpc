    ...
    disconnectFromSocketOnDoneFlag( false )
{
    ...
    if ( dstHost.isEmpty() && socket->state() != QAbstractSocket::ConnectedState )
      {
        qCritical() << this << "constructor, dstHost.isEmpty(), but socket state" << socket->state();
    ...
    connect( connectTimeoutTimer, SIGNAL( timeout()), SLOT( slotSocketConnectTimeout()) );
...
void ClientProtocol::slotSocketStateChanged( QAbstractSocket::SocketState socketState )
{
    ...
    switch ( socketState )
        ...
        case QAbstractSocket::ConnectedState:
            ...
            #ifdef DEBUG_PROTOCOL
            qDebug() << this << "slotSocketStateChanged((): connected!" << "protocolRetry" << protocolRetry;
            #endif
            protocolStart();
            break;
        case QAbstractSocket::UnconnectedState:
            ...
            if ( dstHost.size() )
              {
                #ifdef DEBUG_PROTOCOL
                qDebug() << this << "slotSocketStateChanged(): starting reconnect timer...";
                #endif
                reconnectSleepTimer->start( reconnectSleep );
              } else emitError( unconnectedNoDstHost );
            break;
        default:
            ;   /* do nothing */
        }
...
void ClientProtocol::slotSocketError( QAbstractSocket::SocketError socketError )
{
    ...
    switch ( socketError )
        ...
        case QAbstractSocket::RemoteHostClosedError:
             ...
             * поменяется в UnconnectedState, начнется новый
             * deferredStart
             */
            break;
        case QAbstractSocket::HostNotFoundError:
            qWarning() << this << "slotSocketError(): host not found" << dstHost << ":" << dstPort;
        ...
        case QAbstractSocket::UnknownSocketError:
            qCritical() << this << "slotSocketError(): bad socket error, aborting" << socketError;
            emitError( "bad socket error" );
            break;
        default:
            qCritical() << this << "slotSocketError(): unknown socket error" << socketError;
...
void ClientProtocol::deferredStart()
{
    ...
    switch ( socket->state() )
        ...
        case QAbstractSocket::UnconnectedState:
            ...
            #ifdef DEBUG_PROTOCOL
            qDebug() << this << "deferredStart(): socket->connectToHost(" << dstHost << "," << dstPort << ")";
            #endif
            if ( dstHost.size() ) connectSocket();
            else emitError( "dstHost is empty and socket unconnected" );
            break;
        case QAbstractSocket::ConnectedState:

            /* уже соединены, запускаем протокол */
            #ifdef DEBUG_PROTOCOL
            qDebug() << this << "deferredStart(): connected! protocolRetry" << protocolRetry;
            #endif
            protocolStart();
            break;
        default:
...
{
    qDebug()
    << this << "ClientProtocol::slotBytesWritten(" << bytes << ")";
    #else
    ...
    void ClientProtocol::emitDone()
    {
        ...
        if ( disconnectFromSocketOnDoneFlag ) socket->disconnect( this );
    ...
    void ClientProtocol::protocolStop()
    {
        ...
        disconnect( socket, SIGNAL( bytesWritten( qint64)), this, SLOT( slotBytesWritten( qint64)) );
    }
    ...
    void ClientProtocol::protocolStart()
    {
        ...
        /* noProxyMode? */
        if ( !qstrcmp( metaObject()->className(), "ClientProtocol") ) emitDone();
    }

    ClientProtocolWithTimeout::ClientProtocolWithTimeout( QTcpSocket * parent, const int _protocolTimeout, const QString & dstHost, const quint16 dstPort ) :
    ...
    protocolTimeout( _protocolTimeout )
      {
        ...
        connect( protocolTimeoutTimer, SIGNAL(timeout()), SLOT(slotProtocolTimeout()) );
      }
    void ClientProtocolWithTimeout::protocolStop()
    {
        #ifdef DEBUG_PROTOCOL
        qDebug() << this << "ClientProtocolWithTimeout::protocolStop():" << "stop protocol timeout timer";
        #endif
        ClientProtocol::protocolStop();
        if ( protocolTimeoutTimer->isActive() ) protocolTimeoutTimer->stop();
    }
    ...
    void ClientProtocolWithTimeout::protocolStart()
    {
        #ifdef DEBUG_PROTOCOL
        qDebug()
            << this << "ClientProtocolWithTimeout::protocolStart():" << "start protocol timeout timer" <<
    ...
    void ClientProtocolWithTimeout::slotProtocolTimeout()
    {
        ...
        if ( !objectName().length() )
          {
            qCritical()
                << this << "ClientProtocolWithTimeout::slotProtocolTimeout():" << "protocol name empty!" <<
    ...
    void ClientProtocolWithTimeout::slotBytesWritten( qint64 bytes )
    {
        qDebug()
        << this <<
            "ClientProtocolWithTimeout::slotBytesWritten(" <<
            bytes <<
        ...
        void ClientProtocolWithTimeout::slotReadyRead()
        {
            #ifdef DEBUG_PROTOCOL
            qDebug()
                << this << "ClientProtocolWithTimeout::slotReadyRead():" << "restart protocol timeout timer" <<
                    protocolTimeout / 1000 << "secs";
            #endif
            Q_ASSERT( protocolStarted );
            Q_ASSERT( protocolTimeoutTimer->isActive() );
