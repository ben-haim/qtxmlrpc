...
public:
    ...
    ~   ClientProtocol()    { qDebug() << this << "destructor"; }
        #endif
...
public slots :
     ...
     */
    void deferredStart();
    protected slots : void slotSocketStateChanged( QAbstractSocket::SocketState socketState );
...
protected:
    ...
    /*
     * socket == parent, сделано чтобы не кастить каждый
     * раз парента
     */
    QTcpSocket      *socket;
    QString         dstHost;
    ...
    /*
     * Cыны обязаны вызвать родителей первой
     * строкой
     */
    virtual void    protocolStop();

    /*
     * Может вызываться maxProtocolRetries раз, если в
     * процессе ;
     * работы происходит Disconnect, либо 1 раз, если
     * дали сконнекченный socket
     */
    virtual void    protocolStart();

    /*
     * если дают QSSLSocket, можно перекрыть ;
     * обычное поведение тут (родителя вызывать
     * уже не нужно) ;
...
public:
    ClientProtocolWithTimeout( QTcpSocket *parent, const int _protocolTimeout, const QString &dstHost= QString::null,
        ...
        #ifdef DEBUG_PROTOCOL
    ~   ClientProtocolWithTimeout()     { qDebug() << this << "destructor"; }
        #endif
    protected slots : void slotProtocolTimeout();
